...
class SimpleSynthesizer:
    ...
    def synthesize(
        ...
        if not phoneme_list:
            ...
            return RuntimeExecutionResult(audio=[0.0], state=controller.state, metrics=controller.metrics)
        ...
        for index, token in enumerate(phoneme_list):
            if controller.interrupted:
                controller.finish_generation()
                audio = interleave_runtime_breathing(samples, sentence_length=word_count, emotion=runtime_control.emotion if runtime_control else "neutral")
                return RuntimeExecutionResult(audio=audio, state=controller.state, metrics=controller.metrics, interrupted=True, stop_reason=controller.stop_reason)

            if controller.paused:
                controller.finish_generation()
                audio = interleave_runtime_breathing(samples, sentence_length=word_count, emotion=runtime_control.emotion if runtime_control else "neutral")
                return RuntimeExecutionResult(audio=audio, state=controller.state, metrics=controller.metrics, paused=True, stop_reason=controller.stop_reason)
        ...
        controller.finish_generation()
        audio = interleave_runtime_breathing(samples, sentence_length=word_count, emotion=runtime_control.emotion if runtime_control else "neutral")
        return RuntimeExecutionResult(audio=audio, state=controller.state, metrics=controller.metrics)
